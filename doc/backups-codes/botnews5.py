...
async def searchnews( message ):
    global busca

    busca = True

    await bot.reply_to( message, ' Digite palavra chave ou titulo para pesquisar na lista de notícias.')
    #text = message.text
    #if text != '/busca' and text.find('/') == -1 :
    #    text = text.strip()
    #    df = df[ [ str.lower( news_title .find( str.lower( text )) != -1 for news_title in df['title'] ) ] ]
    #
...
async def echo_all(message):
    ...
    if message.chat.id == '@sofianewsfeed':
        ...
        df = json.loads( df )
        text = ''
        number_of_news = 6 # seis primeiras noticias encontradas.
        for i in df:
            if number_of_news == 0 :
                break
...
async def dailymessage():
    #print('hi')
    datadirs = '/home/opc/news-crawler/datanews/*.json'
    datajsonfiles = glob.glob( datadirs )
...
async def crawlerGetNews():
    ...
    await crawler2.create_list_news("Noticias", 0, True)
